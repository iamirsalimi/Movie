...
export default function ActorsCard({ id, castId, fullName ,role, src }) {
    return (
        <a href={`/actors/${castId}`} className="relative pb-2 group">
            <li key={id} className="group translate-0 transition-all hover:-translate-y-2 overflow-hidden relative rounded-lg h-40 cursor-pointer">
                ...
                <span className="text-nowrap absolute -bottom-1 left-1/2 z-20 -translate-1/2 font-shabnam-light bg-green-500 text-white cursor-pointer px-2 py-0.5 text-xs rounded-sm  font-bold">{role == 'actor' ? 'بازیگر' : role == 'director' ? 'کارگردان' : role == 'voiceActor' ? 'صدا پیشه' : 'نویسنده'}</span>
            </li>
        </a>
    )
}
