...
export default function NewMovieCard({ id, src, title, type , showTitle }) {
    ...
    return (
        <a href={`/${type == 'series' ? 'series' : 'movie'}/${id}`} className="relative pb-2 group">
            ...
            {showTitle && (
                <span className="text-nowrap line-clamp-1 absolute text-sm -bottom-7 left-1/2 -translate-1/2 text-gray-700 dark:text-white  opacity-100 transition-all group-hover:-translate-y-3 group-hover:opacity-0">{title}</span>
            )}
        </a>
    )
}
