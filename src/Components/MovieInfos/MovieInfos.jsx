...
export default function MovieInfos({ infoTitle, infoValue, children }) {
  return (
    <div className="relative bg-white dark:bg-primary border border-gray-200 dark:border-secondary rounded-xl h-fit pt-3 pb-1 px-2">
      ...
      <span className="text-primary dark:text-white font-vazir text-xs sm:text-sm">{typeof infoValue == 'object' ? infoValue.map(value => (
        <span className="group text-sm px-0.5 md:px-1"><span>{value}</span><span className="group-last:hidden text-slate-500">,</span></span>
      )) : infoValue}</span>
    </div>
  )
}
