            ...
            <div className="mx-auto container flex items-center justify-between py-3 px-7">
                    ...
                    <ul className="hidden lg:flex items-center gap-1">
                        {links.map(link => (
                            <a href={link.href} className={`${link.href == location.pathname && 'activeLink'}`} >
                                <li className={`font-vazir p-1 px-2 rounded-lg  ${navFlag ? 'text-light-gray hover:bg-gray-100 dark:hover:bg-primary dark:' : ''}text-white hover:text-sky-400 transition-colors`}>{link.title}</li>
                            </a>
                        ))}
                ...
                <div className="flex items-center justify-end gap-2">
                    ...
                    <div className="flex items-center justify-center gap-2">
                        <a href="/account/login" className={`hidden md:block bg-sky-500 ${navFlag ? 'hover:bg-white dark:' : 'border-transparent '}hover:bg-primary border hover:border-sky-500 transition-colors p-2 rounded-lg hover:text-sky-500 text-white dark:border-secondary font-vazir cursor-pointer`}>حساب کاربری</a>
                        <a href="/account/login" className={`md:hidden  relative w-10 h-10 rounded-full bg-white overflow-hidden`}>
                            <FaUser className="text-gray-500 absolute -bottom-5 left-1/2 -translate-1/2 w-8 h-8" />
                        </a>
                    </div>
                </div>
            </div>
